...
-- ------------------------------------------------------------------------------
-- Create CDM tables from tmp_*, loaded from CSV
-- ------------------------------------------------------------------------------
...
CREATE OR REPLACE TABLE `@bq_target_project.@bq_target_dataset`.{table_name} AS
SELECT
    concept_id,
    concept_name,
    domain_id,
    ...
    invalid_reason
FROM
    `@bq_target_project.@bq_target_dataset`.tmp_{table_name}
;

DROP TABLE IF EXISTS `@bq_target_project.@bq_target_dataset`.tmp_{table_name}
;

field = 
    if 'datetime' in field_name:
